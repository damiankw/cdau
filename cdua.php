...
if (php_sapi_name() === 'cli') {
    echo "cPanel Directory Usage Analyzer v" . APP_VERSION . "\n";
    echo "Running directory analysis...\n\n";
    
    $pathHandler = new PathHandler();
    $scanner = new DirectoryScanner();
    
    // Scan from base directory
    $fullPath = $pathHandler->getBaseDir();
    $contents = $scanner->scanDirectory($fullPath);
    $totalSize = $scanner->calculateTotalSize($contents);
    
    // Generate report
    $report = generateCliReport($contents, $totalSize, $scanner);
    ...
    if (!empty(APP_EMAIL)) {
        $subject = "cPanel Disk Usage Report - " . (isset($_SERVER['SERVER_NAME']) ? $_SERVER['SERVER_NAME'] : 'Server');
        $success = mail(APP_EMAIL, $subject, $report, "Content-Type: text/plain; charset=UTF-8");
        
        if ($success) {
            echo "\n✅ Report emailed to: " . APP_EMAIL . "\n";
        } else {
            echo "\n❌ Failed to send email to: " . APP_EMAIL . "\n";
        }
    } else {
        echo "\n💡 To receive email reports, set APP_EMAIL in the configuration.\n";
    }
 ...
 * Generate CLI report
 */
function generateCliReport($contents, $totalSize, $scanner) {
    $report = "";
    $report .= "=== cPanel Directory Usage Report ===\n";
    $report .= "Generated: " . date('Y-m-d H:i:s') . "\n";
    $report .= "Server: " . (isset($_SERVER['SERVER_NAME']) ? $_SERVER['SERVER_NAME'] : 'Unknown') . "\n";
    $report .= "Total Size: " . $scanner->formatBytes($totalSize) . "\n";
    $report .= "Total Items: " . count($contents) . "\n\n";
    
    $report .= "Top 20 Largest Directories/Files:\n";
    $report .= str_repeat("-", 80) . "\n";
    $report .= sprintf("%-40s %-10s %-12s %s\n", "Name", "Type", "Size", "Modified");
    $report .= str_repeat("-", 80) . "\n";
    ...
    foreach ($contents as $item) {
        ...
        $name = strlen($item['name']) > 38 ? substr($item['name'], 0, 35) . '...' : $item['name'];
    ...
    // Summary by type
    $typeStats = ['directory' => 0, 'file' => 0, 'symlink' => 0];
    $typeSizes = ['directory' => 0, 'file' => 0, 'symlink' => 0];
    ...
    foreach ($typeStats as $type => $count) {
        $report .= sprintf("%-12s: %5d items, %s\n", 
            ucfirst($type), 
            $count, 
            $scanner->formatBytes($typeSizes[$type])
        );
    }
    
    $report .= "\n" . str_repeat("=", 80) . "\n";
    $report .= "Report generated by cPanel Directory Usage Analyzer v" . APP_VERSION . "\n";
    $report .= "Report generated by cPanel Directory Usage Analyzer v" . APP_VERSION . "\n";
    ...
    <div class="container">
        ...
        <div class="footer">
            <div style="text-align: center; padding: 15px; border-top: 1px solid #eee; color: #6c757d;">
                <small>
                    Generated by <?= APP_NAME ?> v<?= APP_VERSION ?> | <a href="https://github.com/damiankw/cdua/" target="_blank" style="color: #007bff;">Open Source</a>
                </small>
            </div>
        </div>
    </div>
